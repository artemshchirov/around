...
Promise.all([api.getUserInfo(), api.getInitialCards()])
  ...
  .catch((err) => console.log(`Error loading user profile and cards: ${err}`));
...
const popupAddCard = new PopupWithForm({
  ...
  handleFormSubmit: (formData) => {
    ...
    api
      ...
      .catch((err) => console.log(`Error create card: ${err}`))
...
const popupCardDelete = new PopupSubmit('.popup_card-delete-confirm', {
    ...
    handleDeleteCard: () => {
      popupCardDelete.setSubmitAction({
        handleSubmitAction: () => {
          ...
          api
            .deleteItem(card.getId())
            .then(() => {
              card.deleteCard();
              popupCardDelete.close();
            })
            .catch((err) => console.log(`Error remove card: ${err}`))
            .finally(() =>
              renderDeleting(false, 'button_form_submit-delete-card')
            );
        },
      });
    ...
    handleChangeLikeStatus: () => {
      if (card.getLikeStatus()) {
        ...
        api
          ...
          .then((cardLiked) => card.updateLike(cardLiked.likes))
          .catch((err) => console.log(`Error remove like: ${err}`));
      }
    },
  });
